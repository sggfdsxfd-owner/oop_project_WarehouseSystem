...
bool Database::loadCargo(Warehouse& warehouse) {
    ...
    while (std::getline(file, line)) {
        ...
        if (type == "General") {
            warehouse.addCargo(std::make_shared<Cargo>(id, name, weight, volume, baseRate, owner));
        } else if (type == "Dangerous" && tokens.size() >= 9) {
            int hazardLevel = std::stoi(tokens[7]);
            std::string unNumber = tokens[8];
            warehouse.addCargo(std::make_shared<DangerousCargo>(id, name, weight, volume, baseRate, owner, hazardLevel, unNumber));
        } else if (type == "Perishable" && tokens.size() >= 9) {
            std::string expiryDate = tokens[7];
            double requiredTemp = std::stod(tokens[8]);
            warehouse.addCargo(std::make_shared<PerishableCargo>(id, name, weight, volume, baseRate, owner, expiryDate, requiredTemp));
        } else if (type == "Fragile" && tokens.size() >= 9) {
            std::string packagingType = tokens[7];
            int maxStackHeight = std::stoi(tokens[8]);
            warehouse.addCargo(std::make_shared<FragileCargo>(id, name, weight, volume, baseRate, owner, packagingType, maxStackHeight));
        }
    }
    file.close();
    return true;
